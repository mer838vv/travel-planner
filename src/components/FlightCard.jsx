...
function ConnectionNote({ connection }) {
  ...
  const text = connection.changesAirport
    ...
    : connection.level === 'critical'
      ? `На пересадку всего ${gap}. Этого почти наверняка не хватит.`
      : connection.level === 'risky'
        ? `На пересадку ${gap}. ${connection.separateTickets
            ? 'Билеты разными бронями — если первый рейс опоздает, пересаживать тебя никто не обязан. На таких стыковках советуют закладывать от трёх часов.'
            : 'Запас небольшой, но билет единый — при опоздании пересадит перевозчик.'}`
        : `На пересадку ${gap} — запас есть.${connection.separateTickets
            ? ' Билеты разными бронями: посадочный на второй рейс получай сам, заранее.'
            : ''}`
